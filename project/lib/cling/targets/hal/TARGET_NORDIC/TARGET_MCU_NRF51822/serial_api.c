...
void serial_baud(serial_t *obj, int baudrate)
{
    ...
    for (int i = 1; i < sizeof(acceptedSpeeds)/sizeof(acceptedSpeeds[0]); i++) {
        if (baudrate<acceptedSpeeds[i][0]) {
            obj->uart->BAUDRATE = acceptedSpeeds[i - 1][1];
            return;
        }
    }
    obj->uart->BAUDRATE = UART_BAUDRATE_BAUDRATE_Baud921600;
}
...
// * INTERRUPT HANDLING
//******************************************************************************
static inline void uart1_irq(uint32_t iir, uint32_t index)
{
    SerialIrq irq_type;
    switch (iir) {
        case 1:
