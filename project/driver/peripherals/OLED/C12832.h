...
protected:
    ...
    void wr_dat(unsigned char value);
		    /**
     * Write data stream to the LCD controller
     *
     * @param dat data pointer written to LCD controller
		 * size:data lenth
     */
		void wr_dat(unsigned char *dat, uint16_t size);
    /**
    ...
    void lcd_set_contrast(oled_color_t type, uint8_t step);
