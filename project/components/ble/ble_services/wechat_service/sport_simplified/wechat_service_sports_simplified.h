...
typedef enum
{
    BLE_UICO_EVT_NOTIFICATION_ENABLED,                             /**< uico value notification enabled event. */
    BLE_UICO_EVT_NOTIFICATION_DISABLED,                             /**< uico value notification disabled event. */
	  BLE_UICO_EVT_BUFFER_SENDED_SUCESSFULLY,
    BLE_UICO_EVT_TEST_TOOL_DISCONNECTED  /*UICO test tool disconnected*/ 
} ble_uico_evt_type_t;
//ble_bas_evt_type_t
/**@brief Battery Service event. */
typedef struct
{
    ble_uico_evt_type_t evt_type;                                  /**< Type of event. */
} ble_uico_evt_t;

// Forward declaration of the ble_bas_t type. 
typedef struct ble_uico_s ble_wechat_sports_t;

/**@brief Battery Service event handler type. */
typedef void (*ble_uico_evt_handler_t) (ble_wechat_sports_t * p_bas, ble_uico_evt_t * p_evt);

/**@brief Battery Service init structure. This contains all options and data needed for
 *        initialization of the service.*/
typedef struct
{
    ble_uico_evt_handler_t        evt_handler;                    /**< Event handler to be called for handling events in the Battery Service. */
...
#define  WECHAT_SPORTS_SOPPURTED_CHAR_COUNT 2
		ble_gatts_char_handles_t      wechat_sports_handles[WECHAT_SPORTS_SOPPURTED_CHAR_COUNT];          /**< Handles related to the Battery Level characteristic. */
    uint8_t												is_char_handle_notify_enabled[WECHAT_SPORTS_SOPPURTED_CHAR_COUNT];	
 ...
 */
uint32_t ble_uico_battery_level_update(ble_wechat_sports_t * p_bas, uint8_t battery_level);


int ble_uico_notify_data_to_pc(ble_wechat_sports_t * p_bas, ble_gatts_char_handles_t handle, uint8_t *buffer, uint16_t lenth);
int ble_uico_recieve_data_from_pc_register( int (*p)(uint8_t *, uint16_t));
uint16_t  wechat_sports_update_deamon(void);   
#endif // BLE_BAS_H__
