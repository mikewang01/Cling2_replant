 ...
 */
static bool twi_master_clear_bus()
{
	uint32_t twi_state;
	bool     bus_clear;
  ...
  NRF_TWI1->ENABLE = TWI_ENABLE_ENABLE_Disabled << TWI_ENABLE_ENABLE_Pos;
  clk_pin_config   = NRF_GPIO->PIN_CNF[TWI_MASTER_1_CONFIG_CLOCK_PIN_NUMBER];
  data_pin_config  = NRF_GPIO->PIN_CNF[TWI_MASTER_1_CONFIG_DATA_PIN_NUMBER];
  
  GPIO_twi_output_config();
  
  TWI_1_SDA_HIGH();
  TWI_1_SCL_HIGH();
  TWI_DELAY();
  
  if ((TWI_1_SDA_READ() == 1) && (TWI_1_SCL_READ() == 1)) {
  	bus_clear = true;
  } else {
  	uint_fast8_t i;
  	bus_clear = false;
  ...
  NRF_GPIO->PIN_CNF[TWI_MASTER_1_CONFIG_CLOCK_PIN_NUMBER] = clk_pin_config;
  NRF_GPIO->PIN_CNF[TWI_MASTER_1_CONFIG_DATA_PIN_NUMBER]  = data_pin_config;
...
bool twi_master_init()
{
  ...
  NRF_TWI1->ENABLE = 0;
  GPIO_twi_init_config();
  NRF_TWI1->EVENTS_RXDREADY = 0;
  NRF_TWI1->EVENTS_TXDSENT = 0;
  NRF_TWI1->PSELSCL = TWI_MASTER_1_CONFIG_CLOCK_PIN_NUMBER;
  NRF_TWI1->PSELSDA = TWI_MASTER_1_CONFIG_DATA_PIN_NUMBER;
